...
def select_page():
    data = farmers_tb_select()
    # st.write(data)
    n = 1
    for i in data:
        col1, col2, col3 = st.columns([1,2,2])
        with col1:
            st.subheader("{}. {:03d}".format(n,i[0]))
        with col2:
            ...
            st.text("หมู่ทึ่: {}".format(i[14]))
            st.text("ตำบล: {}".format(i[16]))
            st.text("จังหวัด: {}".format(i[18]))
            st.text("การตรวจวิเคราะห์ดิน: {}".format(i[23]))
            st.text("พิกัด X: {}".format(i[19]))
            st.text("พิกัด Z: {}".format(i[21]))
        with col3:
            st.caption(".")
            ...
            st.text("รหัสไปรษณีย์: {}".format(i[13]))
            st.caption(".")
            st.text("ชื่อหมู่บ้าน: {}".format(i[15]))
            st.text("อำเภอ: {}".format(i[17]))
            st.text("สิทธิการใช้ประโยชน์ที่ดิน: {}".format(i[22]))
            st.text("การรับรองระบบ GAP: {}".format(i[25]))
            st.text("การตรวจวิเคราะห์น้ำ: {}".format(i[24]))
            st.text("พิกัด Y: {}".format(i[20]))
